...
/**
 * This is meant to be a base [RecyclerView.ItemAnimator] that has no implementation but is used
 * as a framework to manage and simplify animations for subclasses
 */
abstract class BaseItemAnimator : SimpleItemAnimator() {

    protected val interpolator = LogarithmicInterpolator()

    protected val pendingRemovals = ArrayList<ViewHolder>()
    protected val pendingAdditions = ArrayList<ViewHolder>()
    protected val pendingMoves = ArrayList<MoveInfo>()
    protected val pendingChanges = ArrayList<ChangeInfo>()

    // TODO: 08-Jun-20 Why do we have double nested lists??
    protected val additionsList = ArrayList<ArrayList<ViewHolder>>()
    protected val movesList = ArrayList<ArrayList<MoveInfo>>()
    protected val changesList = ArrayList<ArrayList<ChangeInfo>>()

    protected val addAnimations = ArrayList<ViewHolder>()
    protected val moveAnimations = ArrayList<ViewHolder>()
    protected val removeAnimations = ArrayList<ViewHolder>()
    protected val changeAnimations = ArrayList<ViewHolder>()
    ...
    override fun runPendingAnimations() {
        ...
        if (movesPending) {
            ...
            pendingMoves.clear()
            val mover: () -> Unit = {
                moves.forEach { animateMoveImpl(it.holder, it.fromX, it.fromY, it.toX, it.toY) }
                moves.clear()
